...
interface CaregiverSidebarShellProps {
  children: ReactNode;
}
...
export default function CaregiverSidebarShell({ children }: CaregiverSidebarShellProps) {
  ...
  const navigationItems: Array<{
    ...
    { label: 'Sleep Analysis', icon: BarChart2, path: '/caregiver/sleep-analytics', action: null },
    { label: 'My Progress', icon: TrendingUp, path: '/caregiver/progress', action: null },
    { label: 'Messages', icon: MessageCircle, path: '/caregiver/messages', action: null },
    { label: 'Reminders', icon: Bell, path: '/caregiver/reminders', action: null, badge: activeCount },
  ];
      ...
      >
        ...
        <div className="mt-auto p-4 space-y-2 shrink-0" style={{ borderTop: `0.5px solid ${token.purple100}` }}>
          <button
            ...
            className="w-full flex items-center justify-start space-x-3 rounded-xl transition-all duration-200 hover:bg-[#F3E8FF]"
            style={navButtonStyle(location.pathname === '/caregiver/settings')}
          >
            <Settings size={18} strokeWidth={1.5} color={location.pathname === '/caregiver/settings' ? '#6D28D9' : '#6B7280'} className="flex-shrink-0" />
